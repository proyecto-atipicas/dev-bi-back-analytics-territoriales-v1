...
export interface FiltroSocioeconomico {
  ...
  /** Antes `ano`. */
  periodo?: number | null;
  /** Nuevo: filtra por `referencia`. */
  referencia?: string | null;
  /** Nuevo: filtra por `nivel_geografico` (Departamental/Nacional/…). */
...
export interface SocioeconomicoRepositoryPort {
  ...
  /** Distinct de `referencia`. Acepta filtro por fuente, fuentePublicacion y dimensión. */
  listarReferencias(filtro: FiltroSocioeconomico): Promise<string[]>;

  /** Distinct de `nivel_geografico`. Acepta filtro por fuente, fuentePublicacion, dimensión y referencia. */
   ...
   * respetando los demás filtros (dimensión, fuente, etc.). Sirve para el
   ...
   * dimensiones de la fuente). Sirve para el panel "Detalle por departamento".
   */
